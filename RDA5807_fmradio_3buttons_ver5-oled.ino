...
void visualPI(void)
{
     ...
     tft.setCursor(1,47);sprintf(buffer,"COUNTRY -%02d",RDS[0]>>12 & 0X000F);tft.setTextColor(WHITE, BLACK); tft.setTextSize(1); tft.print(buffer);tft.print(" "); 
     tft.setCursor(1,47);sprintf(buffer,"COVERAGE-%02d",RDS[0]>>8 & 0X000F);tft.setTextColor(WHITE, BLACK);tft.setTextSize(1); tft.print(buffer);
     tft.setCursor(1,57);sprintf(buffer,"CODE    -%02d",RDS[0] & 0X00FF); tft.setTextColor(WHITE, BLACK);tft.setTextSize(1);tft.print(buffer);      
 delay(1000);
tft.clearDisplay();
}
void visualPTY(void)
{
     ...
     tft.setCursor(1,10);   tft.setTextColor(WHITE, BLACK);  tft.print("TYPE"); 
     tft.setCursor(1,20);  tft.setTextColor(WHITE, BLACK);    tft.print("PROGRAMS");
     tft.setCursor(1,40);sprintf(buffer,"%02d",RDS[1]>>5 & 0X001F); tft.setTextColor(WHITE, BLACK); tft.print(buffer);
     delay(2000);
tft.clearDisplay();
}
...
void clearRDS(void)
{       
        tft.setCursor(20,47); for (z=0;z<8;z++) {seg_RDS[z]=32; tft.setTextColor(WHITE, BLACK);}  //borrar Name LCD Emisora
        tft.setCursor(20,57); for (z=0;z<6;z++) { tft.setTextColor(WHITE, BLACK);}  //borrar linea Hora
...
{ 
    ...
    // tft.setCursor(27, 20); tft.setTextSize(1); tft.setTextColor(YELLOW, BLACK); tft.print("-Radio-"); 
      sprintf(buffer,"%s",menuS[menu]); tft.setCursor(0,35); tft.setTextSize(1); tft.setTextColor(WHITE,BLACK); tft.print(buffer); 
       //Detectar se&#241;al stereo
     //  tft.setCursor(85,8);
     //  tft.setTextColor(WHITE, BLACK);
     //  if((estado[0] & 0x0400)==0)  tft.print("");   else     tft.setTextColor(WHITE, BLACK); tft.print("Stereo");  tft.setTextColor(WHITE, BLACK);      
       //Se&#241;al 
     //  z=estado[1]>>10; sprintf(buffer,"S-%02d",z); tft.setCursor(58,0); tft.setTextColor(GREEN, BLACK); tft.print(buffer); tft.setCursor(25,0);tft.setTextColor(GREEN, BLACK);tft.print("Canal");
      ...
      sprintf(buffer,"Volum %02d",volumen); tft.setCursor(70,35);tft.setTextSize(1); tft.setTextColor(WHITE, BLACK); tft.print(buffer); //DUBLARE AFISARE VOLUM
       tft.display();
       //tft.print("/15");
       //ver RADIO_TXT pozitia rds
       tft.setCursor(20,47);tft.setTextColor(WHITE, BLACK);
       z1=indexRDS1;
       for (z=0;z<12;z++)
       {           
  ...
  grupo=(RDS[1]>>12)&0xf;
    ...
    switch(grupo)
    {
     case 0:              
      ...
      tft.setCursor(0,57);tft.setTextColor(WHITE, BLACK); 
      for (i=0;i<8;i++)
      {
        #if DEBUG 
